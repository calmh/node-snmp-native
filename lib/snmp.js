...
// Shortcut to create a GetBulkRequest and send it, while registering a callback.
// Needs `options.oid` to be an OID in array form.

Session.prototype.getBulk = function (options, callback) {
    var self = this, pkt;
    ...
    pkt.community = options.community;
    pkt.version = options.version;
    // For GetBulkRequest PDUs the error and errorIndex are overridden
    // and represent the non-repeaters and max-repetitions variables
    // non-repeaters is the number of varbinds which are not repeated
    ...
    function result(error, varbinds) {
        ...
        if (error) {
            ...
            for (var i = 0; i < varbinds.length; i++) {
                var vb = varbinds[i];
                if ( !(inTree(options.startOid, vb.oid)) ||
                    vb.value === 'endOfMibView' ||
                    vb.value === 'noSuchObject' ||
                    vb.value === 'noSuchInstance') {
                        clearTimeout(combinedTimeoutTimer);
                    ...
                    ( i === 0 && vbs.length &&
                      compareOids(vbs.slice(-1)[0].oid, varbinds[0].oid) !== 1) ||
                    ( i !== 0 && compareOids(varbinds[i-1].oid, varbinds[i].oid) !== 1) ) {
                        clearTimeout(combinedTimeoutTimer);
                        return callback(new Error('OID not increasing', vbs));
                } else {
                    vbs.push(vb);
                    next = { oid: vb.oid };
                }
