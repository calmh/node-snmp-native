...
function parseOids(options) {
    if (options.oid && typeof options.oid === 'string') {
        options.oid = _.map(_.compact(options.oid.split('.')), function (x) { return parseInt(x, 10); });
    }

    if (options.oids) {
        options.oids = _.map(options.oids, function (oid) {
            if (typeof oid === 'string') {
                return _.map(_.compact(oid.split('.')), function (x) { return parseInt(x, 10); });
            } else {
                return oid;
            }
        });
...
function msgReceived(msg, rinfo) {
    ...
    if (entry) {
        ...
        entry.cb(null, pkt.pdu.varbinds);
    } else {
...
Session.prototype.requestId = function () {
    ...
    return (now << 10 & 0x7fffffff) + self.counter;
};
...
Session.prototype.sendMsg = function (pkt, options, callback) {
    ...
    function transmit() {
        ...
        if (DEBUG_RETRANSMIT && retrans > 1) {
            ...
            console.warn('Retransmit #' + retrans + ' for reqid ' + reqid + ' oid ' + pkt.pdu.varbinds[0].oid + ' to ' + self.options.host);
        }
        ...
        self.socket.send(buf, 0, buf.length, options.port, options.host, function (err, bytes) {
            if (err) {
                ...
                if (options.timeouts[retrans]) {
                    timeOut = setTimeout(transmit, options.timeouts[retrans]);
