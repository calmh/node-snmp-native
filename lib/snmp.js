...
var RETRANSMISSIONS = [ 2500, 2500, 5000, 5000, 5000 ];

// Set this to true to get debug output for retransmissions.
var DEBUG_RETRANSMIT = false;
...
function requestId() {
    ...
    if (!requestId.prevTs) {
        requestId.prevTs = now;
        requestId.counter = 0;
    }

    if (now === requestId.prevTs) {
        requestId.counter += 1;
        if (requestId.counter > 1023) {
    ...
    return (now << 10 & 0x7fffffff) + requestId.counter;
}
...
function msgReceived(msg, rinfo) {
    var self = this, now = Date.now(), pkt, cb, timeOut, entry;
...
Session.prototype.sendMsg = function (pkt, destination, callback) {
    ...
    if (arguments.length === 2) {
        ...
        throw new Error('Called with incorrect number of arguments');
    }
    ...
    function transmit() {
        ...
        if (DEBUG_RETRANSMIT && retrans > 1) {
            ...
            console.warn('Retransmit #' + retrans + ' for reqid ' + reqid + ' oid ' + pkt.pdu.varbinds[0].oid + ' to ' + self.destination);
        }
        ...
        self.socket.send(buf, 0, buf.length, 161, destination, function (err, bytes) {
...
Session.prototype.get = function (oid, destination, community, callback) {
    var self = this, pkt;
    ...
    pkt = new Packet();
    pkt.community = community;
    pkt.pdu.varbinds[0].oid = oid;
    self.sendMsg(pkt, destination, callback);
};

// Shortcut to create a SetRequest with an integer and send it, while registering a callback.

Session.prototype.setInteger = function (oid, val, destination, community, callback) {
    var self = this, pkt;
    ...
    pkt.pdu.varbinds[0].oid = oid;
    pkt.pdu.varbinds[0].type = asn1ber.types.Integer;
    pkt.pdu.varbinds[0].value = val;
    self.sendMsg(pkt, destination, callback);
};

// Shortcut to get all OIDs in the `oids` array sequentially. The callback
// is called when the entire operation is completed.
...
Session.prototype.getAll = function (oids, destination, community, callback, abortOnErrors) {
    var self = this, pkts = [];
    ...
    function getOne(c) {
        ...
        // Push up to 20 varbinds in the same message.
        // The number 20 isn't really that magical, it's just a nice round
        // number that usually seems to fit withing a single packet and gets
        // accepted by the switches I've tested it on.
        for (m = 0; m < 20 && c < oids.length; m++) {
            vb = new VarBind();
            vb.oid = oids[c];
            pkt.pdu.varbinds.push(vb);
            c++;
        ...
        self.sendMsg(pkt, destination, function (err, data) {
            if (abortOnErrors && err) {
                callback(err);
            } else {
                if (data) {
                    _.each(data.pdu.varbinds, function (vb) {
                        vb.receiveStamp = data.receiveStamp;
                        vb.sendStamp = data.sendStamp;
                    });
                    pkts = pkts.concat(data.pdu.varbinds);
                }
                if (c < oids.length) {
                    getOne(c);
                } else {
...
Session.prototype.getNext = function (oid, destination, community, callback) {
    var self = this, pkt;
...
Session.prototype.getSubtree = function (oid, destination, community, callback) {
    var self = this, vbs = [];
    ...
    function result(error, pkt) {
        if (error) {
            callback(error);
        } else {
            if (inTree(self.startOid, pkt.pdu.varbinds[0].oid)) {
                if (pkt.pdu.varbinds[0].value === 'endOfMibView' || pkt.pdu.varbinds[0].value === 'noSuchObject' || pkt.pdu.varbinds[0].value === 'noSuchInstance') {
                    callback(null, vbs);
                } else {
                    vbs.push(pkt.pdu.varbinds[0]);
                    self.getNext(pkt.pdu.varbinds[0].oid, destination, community, result);
                }
            } else {
                callback(null, vbs);
            }
    ...
    self.startOid = oid;
    self.getNext(oid, destination, community, result);
};
