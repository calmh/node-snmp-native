  ...
  if (entry) {
    ...
    if (typeof entry.callback === 'function') {
      if (pkt.pdu.error !== 0) {
        ...
        var errorDescr =
          Object.keys(asn1ber.errors).filter(function(key) {
...
Session.prototype.getAll = function(options, callback) {
  var self = this,
    results = [],
    combinedTimeoutTimer = null,
    combinedTimeoutExpired = false;
  ...
  function result(error, varbinds) {
    ...
    if (error) {
      ...
      if (inTree(options.startOid, varbinds[0].oid)) {
        if (
          varbinds[0].value === 'endOfMibView' ||
          varbinds[0].value === 'noSuchObject' ||
          varbinds[0].value === 'noSuchInstance'
        ) {
          clearTimeout(combinedTimeoutTimer);
          callback(null, vbs);
        } else if (
