...
Session.prototype.setInteger = function (oid, val, callback) {
    var self = this, pkt;
...
Session.prototype.getAll = function (oids, callback, abortOnErrors) {
    var self = this, pkts = [];
    ...
    function getOne(c) {
        ...
        self.sendMsg(pkt, function (err, data) {
...
Session.prototype.getSubtree = function (oid, callback) {
    var self = this, vbs = [];
        ...
        if (error) {
            ...
            if (inTree(self.startOid, pkt.pdu.varbinds[0].oid)) {
                if (pkt.pdu.varbinds[0].value === 'endOfMibView' || pkt.pdu.varbinds[0].value === 'noSuchObject' || pkt.pdu.varbinds[0].value === 'noSuchInstance') {
                    ...
                    self.getNext(pkt.pdu.varbinds[0].oid, result);
                }
            } else {
                callback(null, vbs);
            }
