...
function parseSingleOid(oid) {
	...
	oid = oid.split('.').filter(function(s) {
		return s.length > 0;
	}).map(function(s) {
...
function parse(buf) {
	...
	while (buf[0] === asn1ber.types.Sequence) {
		...
		} else if (vb.type === asn1ber.types.Integer || vb.type === asn1ber.types.Counter || vb.type === asn1ber.types.Counter64 || vb.type === asn1ber.types.TimeTicks || vb.type === asn1ber.types.Gauge) {
